...
function Integration({ totalQuotation, decodedToken }) {
    ...
    console.log('getServiceRequestData', getServiceRequestData)

    return (
        <>
            <TitleCard title="Approved Quotes" topMargin="mt-2" TopSideButtons={<TopSideButtons searchTextDataFun={searchTextDataFun} ClearData={ClearData} setSearchText={setSearchText} searchText={searchText} />}>
                    ...
                    <table className="table w-full">
                        <thead>
                            <tr className="">
                                ...
                                <th className="text-center bg-cyan600 text-blue font-poppins not-italic font-md">User Status</th>
                                <th className="text-center bg-cyan600 text-blue font-poppins not-italic font-md">Pickup Location</th>
                                <th className="text-center bg-cyan600 text-blue font-poppins not-italic font-md">Drop Location</th>
                                {
                                    (decodedToken?.role !== "Admin") && (
                                        <th className="text-center bg-cyan600 text-blue font-poppins not-italic font-md">Your Status</th>
                                    )
                                }
                                ...
                                getServiceRequestData?.length > 0 ? (
                                    getServiceRequestData?.map((item, index) => (
                                        <tr>
                                            ...
                                            <td className="text-center text-blue font-poppins not-italic font-md dark:text-white">
                                                <div className='w w-24'>{item?.serviceInfo?.pickupLocation ? item?.serviceInfo?.pickupLocation : '---'}</div>
                                            </td>
                                            <td className="text-center text-blue font-poppins not-italic font-md dark:text-white">
                                                <div className='w w-24'>{item?.serviceInfo?.dropLocation ? item?.serviceInfo?.dropLocation : '---'}</div>
                                            </td>

                                            {
                                                (decodedToken?.role !== "Admin") && (
                                                    <td className="text-center text-blue font-poppins not-italic font-md dark:text-white">
                                                        {
                                                            (item?.vendorStatus === "Pending" || item?.vendorStatus === "Running" || item?.vendorStatus === "Completed") ? <p className={`text-white ${(item?.vendorStatus === "Pending") ? ('bg-orange-300') : (item?.vendorStatus === "Running") ? ('bg-sky-600') : (item?.vendorStatus === "Completed") ? ('bg-green-600') : ('')} px-2 rounded-sm`}>
                                                                {(item?.vendorStatus === "Pending") ? (item?.vendorStatus) : (item?.vendorStatus === "Running") ? (item?.vendorStatus) : (item?.vendorStatus === "Completed") ? (item?.vendorStatus) : ('')}
                                                            </p> : ('---')
                                                        }
                                                    </td>
                                                )
